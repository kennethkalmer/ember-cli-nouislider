...
const CREATE_OPTIONS = ['start', 'step', 'margin', 'padding', 'limit', 'range', 'connect', 'orientation', 'direction', 'behaviour', 'animate', 'snap', 'pips', 'format', 'tooltips', 'keyboardSupport', 'cssPrefix', 'cssClasses'];
const UPDATEABLE_OPTIONS = ['margin', 'limit', 'padding', 'range', 'animate', 'snap', 'step', 'format'];
...
class RangeSliderModifier extends Modifier {
  slider = null;
  sliding = false;
  failed = false;
  lastArgs = null;
  ...
  #setup(element, args) {
    ...
    for (const event of EVENTS) {
      this.slider.on(event, (values, handle, unencoded, tap, positions) => this.#dispatch(event, values, handle, unencoded, tap, positions));
    }
...
export { RangeSliderModifier as default };
